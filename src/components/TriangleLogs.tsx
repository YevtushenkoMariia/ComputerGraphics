...
import type { TriangleLogsProps } from "../types/props";

export default function TriangleLogs({
  triangleLogsData,
}: {
  triangleLogsData: TriangleLogsProps[];
}) {
  const [triangleLogs, setTriangleLogs] =
    useState<TriangleLogsProps[]>(triangleLogsData);
  ...
  return (
    ...
    >
      ...
      <div className="overflow-y-auto h-full max-h-[calc(100vh-160px)]">
        {triangleLogs &&
          triangleLogs.map((triangleLog) => (
            <div
              key={triangleLog.name}
              style={{ borderColor: COLORS.darkBlue }}
            >
              <div
                className="w-auto h-fit items-start flex-col justify-start 
              ...
              >
                ...
                <p>
                  ...
                  <small
                    ...
                    style={{ color: triangleLog.borderColor }}
                  >
                    {" "}
                    {triangleLog.borderColor}
                  </small>
                </p>
                <p>
                  Колір заливки:
                  <small
                    className="text-xs text-gray-500"
                    style={{ color: triangleLog.fillColor }}
                  >
                    {" "}
                    {triangleLog.fillColor}
                  </small>
                </p>
                <p>
                  Тип вершини:
